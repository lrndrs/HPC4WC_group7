...
def new_reportfile(report_name):
    ...
    with open(report_name, "w") as newFile:
        ...
        newFileWriter.writerow(
            ["stencil type", "backend", "nx", "ny", "nz", "Elapsed Time", "Validation"]
        )
...
def append_row(
    report_name, stencil_type, backend, nx, ny, nz, elapsedtime, valid_var
):
    """
    ...
    with open(report_name, "a") as newFile:
        newFileWriter = csv.writer(newFile)
        newFileWriter.writerow(
            [stencil_type, backend, nx, ny, nz, elapsedtime, valid_var]
        )
